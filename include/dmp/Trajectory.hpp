...
/** \brief A class for storing trajectories: positions, velocities and
accelerations of
 *  variables over time.
 *
...
t_T   x^0_T x^1_T .. x^D_T   xd^0_T xd^1_T .. xd^D_T   xdd^0_T xdd^1_T ...xdd^D_T
 \endverbatim For each time t_0 to t_T it contains the positions,
velocities and accelerations of a D-dimensional state.
 */
class Trajectory {
 public:
  /** Default constructor.
   ...
   * \param[in] misc Miscellaneous other variables which you may add if
   * necessary
   */
  Trajectory(const Eigen::VectorXd& ts, const Eigen::MatrixXd& ys,
   ...
   * This can be used for instance to store task paramaters for each time step
   * alongside the values of the trajectory variables at each time step. \return
   * Matrix of miscellaneous variables, of size n_time_steps X n_misc_variables
   */
  inline const Eigen::MatrixXd& misc(void) const { return misc_; }

  /** Set miscellaneous variables included in the trajectory.
   * This can be used for instance to set task paramaters for each time step
   * alongside the values of the trajectory variables at each time step.
   * \param[in] misc Matrix of miscellaneous variables, of size n_time_steps X
   * n_misc_variables
   */
  ...
  /** Append another trajectory at the end. The appended trajectory should begin
   * as current trajectory ends, in terms of time, position, velocity an
   * acceleration.
   *
   ...
   * \param[in] overwrite Whether to overwrite existing files (true=overwrite,
   * false=give warning) \return true if writing was successful, false
   * otherwise.
   */
  bool saveToFile(std::string directory, std::string filename,
  ...
  // friend std::istream& operator>>(std::istream& input, Trajectory&
  // trajectory);

  /** Read a trajectory from a file.
   *
   *  \param[in] filename The name of the file from which to read.
   *  \param[in] n_dims_misc Number of miscellaneous variables. This is needed
   * because the file contains a n_time_steps x N matrix, and we need to know
   * which part of M represents the miscellaneous variables, and which part
   * represents the trajectory. \return Trajectory that was read
   *
   ...
   */
  static Trajectory generateMinJerkTrajectory(const Eigen::VectorXd& ts,
                                              const Eigen::VectorXd& initial_y,
                                              const Eigen::VectorXd& final_y);

  /** Generate a fifth order polynomial trajectory from an initial to a final
   * state.
   *
   * \param[in] ts Times at which the state should be computed.
   ...
   */
  static Trajectory generatePolynomialTrajectory(
      const Eigen::VectorXd& ts, const Eigen::VectorXd& y_from,
      const Eigen::VectorXd& yd_from, const Eigen::VectorXd& ydd_from,
      const Eigen::VectorXd& y_to, const Eigen::VectorXd& yd_to,
      const Eigen::VectorXd& ydd_to);

  /** Generate a trajectory from an initial to a final state, through a
   * viapoint.
   *
   * \param[in] ts Times at which the state should be computed.
   * \param[in] y_from Initial position of the trajectory.
   * \param[in] y_yd_ydd_viapoint Viapoint, should contain, pos, vel and acc.
   * \param[in] viapoint_time Time at which the trajectory should pass through
   * the viapoint. \param[in] y_to Final position of the trajectory. \return The
   * trajectory from y_from to y_to, through the viapoint
   */
  static Trajectory generatePolynomialTrajectoryThroughViapoint(
      const Eigen::VectorXd& ts, const Eigen::VectorXd& y_from,
      const Eigen::VectorXd& y_yd_ydd_viapoint, double viapoint_time,
      const Eigen::VectorXd& y_to);
