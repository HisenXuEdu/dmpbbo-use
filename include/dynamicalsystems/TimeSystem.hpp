...
/** \brief Dynamical System modelling the evolution of a time: \f$\dot{x} =
 * 1/\tau\f$. In this system, time is modeled as a 1-D variable that increases
 * with a constant velocity. As with time, the system starts at 0. However,
 * whereas time increases with 1s/s the system increases with (1/tau)/s. Once
 * the system goes above tau, velocities are 0. This system can also model a
 * countdown timer. In this case, the system starts at 1, decreases with
 * velocity -1/tau, and stops decreasing when the system drops below 0.
 *
...
class TimeSystem : public DynamicalSystem {
 public:
   ...
   * DynamicalSystem::tau() \param count_down       Whether timer increases
   * (false) or decreases (true)
   */
   ...
   */
  inline bool count_down(void) const { return count_down_; }
