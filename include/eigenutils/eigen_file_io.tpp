    ...
    Eigen::Matrix<Scalar, RowsAtCompileTime, ColsAtCompileTime>& m)
{
  ...
  m = Eigen::Matrix<Scalar, RowsAtCompileTime, ColsAtCompileTime>(n_rows,
                                                                  n_cols);
    ...
    bool overwrite)
{
  ...
  if (!boost::filesystem::exists(directory)) {
    ...
    if (!boost::filesystem::create_directories(directory)) {
      std::cerr << "Couldn't make directory '" << directory
    ...
    bool overwrite)
{
  if (boost::filesystem::exists(filename)) {
    if (!overwrite) {
      ...
      std::cerr << "File '" << filename << "' already exists. Not saving data."
                << std::endl;
      return false;
    }
  }
