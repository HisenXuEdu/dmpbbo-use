...
def main():
    ...
    if args.task is not None:
        with open(args.task, "r") as f:
            task = jsonpickle.decode(f.read())       #这里task一直没有解析成功是因为TaskThrowBall中task模块找不到，所以decode直接返回dict了。傻逼包竟然不给警告。
    
    print("Object type is: ", type(task))
    ...
    for filename in cost_vars_filenames:
        ...
        #t=TaskThrowBall.TaskThrowBall()
        print(task)
        task.plot_rollout(cost_vars, ax)

    if args.save:
        filename = Path('./', "plot_rollouts.png")
